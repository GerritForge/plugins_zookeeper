...
public class ZkSharedRefDatabaseTest implements RefFixture {
  ...
  private StringToGenericDeserializerFactory stringToGenericDeserializerFactory =
      new StringToGenericDeserializerFactory(asDynamicSet());

  private DynamicSet<StringToGenericDeserializer> asDynamicSet() {
    DynamicSet<StringToGenericDeserializer> result = new DynamicSet<>();
  ...
  public void setup() {
    ...
    zkSharedRefDatabase =
        new ZkSharedRefDatabase(
            ...
            stringToGenericDeserializerFactory);
  }
  ...
  public void shouldReturnObjectIdValueIfExists() throws Exception {
    ...
    zookeeperContainer.createRefInZk(A_TEST_PROJECT_NAME_KEY, ref);

    assertThat(
            zkSharedRefDatabase
                .get(A_TEST_PROJECT_NAME_KEY, ref.getName(), ObjectId.class)
                .isPresent())
        .isTrue();
    assertThat(
            zkSharedRefDatabase.<ObjectId>get(
                A_TEST_PROJECT_NAME_KEY, ref.getName(), ObjectId.class))
        .isEqualTo(Optional.of(AN_OBJECT_ID_1));
  }
